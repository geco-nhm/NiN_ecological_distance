library("readxl")
library("writexl")
setwd("C:/Users/adamen/Documents/Doktorgrad/Artikkel 1/")

#Making vectors for different NiN groups
#Major type groups
firm <- 1:45
wet <- 46:58
limn <- 59:60
...
#Uploading lists with the mapping units and their midpoints along gradients and specifications for principles
MT_sheet <- "ED/HT5.xlsx"
bT_sheet <- "ED/bT5.xlsx"
sLKM_sheet <- "ED/sLKM5.xlsx"
#1:20 000
MT20_sheet <- "ED/HT20.xlsx"
bT20_sheet <- "ED/bT20.xlsx"
sLKM20_sheet <- "ED/sLKM20.xlsx"
n <- 60
MT_list <- numeric()
bT_list <- numeric()
sLKM_list <- numeric()
...
compute_ED <- function(ED_matrix,criteria_matrix,ED_list,between_major_types,between_all_units)
{
        ...
        if (ED_list[[l]][j,i] == 0 || ED_list[[m]][k,i] == 0) {""}
          ...
          { if (m != l)
          {
            if (ED_list[[l]][j,i] == 0 || ED_list[[m]][k,i] == 0) {""}
              ...
              ED_matrix[[(l-1)*n+m]][j,k] <- floor(abs(sum(ED_list[[l]][j,i]) - sum(ED_list[[m]][k,i]))) + ED_matrix[[(l-1)*n+m]][j,k] #beregner ØA innenfor hovedtyper vha HT
              if (floor(abs(sum(ED_list[[l]][j,i]) - sum(ED_list[[m]][k,i]))) > 0){                  
                if (criteria_matrix[[(l-1)*n+m]][[1]][j,k,1] == colnames(ED_list[[l]][i]) ||
                    criteria_matrix[[(l-1)*n+m]][[1]][j,k,2] == colnames(ED_list[[l]][i]) ||
                    criteria_matrix[[(l-1)*n+m]][[1]][j,k,3] == colnames(ED_list[[l]][i]) ||
